...
<script>
    $(document).ready(function () {
        ...
        $('#approve_purchase').on('show.bs.modal', function (event) {
            ...
            if (@json($dataPurchaseItem).length > 0) {
                const optPurchaseAlocation = {
                    initEmpty: true,
                    show: function () {
                        $(this).slideDown();
                        // Feather Icons
                        ...
                        const productItemId = $(this).find('.alocation-product-id').val();
                        const initialValue = parseInt($(`#item-qty-${productItemId}`).val());

                        $(this).find(`.alocation-qty-${productItemId}`).keyup(function (e) { 
                            let totalEntered = 0;
                            $(`.alocation-qty-${productItemId}`).each(function () {
                                totalEntered += parseInt($(this).val().replace(/\./g, '').replace(/,/g, '.')) || 0; // Sum all input values
                            });
                            
                            let newRemain = initialValue - totalEntered;
                            // if (newRemain < 0) newRemain = 0; // Prevent negative values
                            $(`#remain-qty-${productItemId}`).text(newRemain);
                        });
                    },
                    hide: function (deleteElement) {
                        if (confirm('Apakah kamu yakin ingin menghapus data ini?')) {
                            ...
                            setTimeout(function () {
                                let totalEntered = 0;
                                $inputQty = $(`.alocation-qty-${productItemId}`);
                ...
                dataPurchaseItem.forEach(element => {
                    const $alocationRepeater = $(`#purchase-alocation-repeater-${element.product_id}`).repeater(optPurchaseAlocation);
                    if (element.purchase_item_alocation.length > 0) {
                        const itemAlocations = element.purchase_item_alocation;
                        $alocationRepeater.setList(itemAlocations);
                        $('.alocation-input').trigger('keyup');
                        for (let i = 0; i < itemAlocations.length; i++) {
                            $(`select[name="purchase_alocation[${element.product_id}][${i}][warehouse_id]"]`).append(`<option value="${itemAlocations[i].warehouse_id}" selected>${itemAlocations[i].warehouse.name}</option>`);
                            $(`select[name="purchase_alocation[${element.product_id}][${i}][warehouse_id]`).trigger('change');
                        }
                    } else {
                        $(`#add-btn-alocation-${element.product_id}`).trigger('click');
                    }
                });
            } 
        });
