...
@section('content')
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="card">
                                ...
                                <div class="card-body">
                                    <form class="form form-horizontal" method="post" action="{{ route('data-master.product-sub-category.edit', $data->product_sub_category_id) }}">
                                        {{ csrf_field() }}
                                        <div class="row">
                                            <div class="col-12">
                                                    ...
                                                    <div class="col-sm-5">
                                                        <select name="product_category_id" id="product_category_id" class="form-control">
                                                            @if($data->product_category_id && ($data->product_category->name??false))
                                                                <option value="{{ $data->product_category_id }}" selected="selected">{{ $data->product_category->category_code??''}} - {{$data->product_category->name??'' }}</option>
                                                            @endif
                                                        </select>
                    ...
                    <script>
                        $(document).ready(function() {
                            ...
                            var oldValue = "{{ $data->product_category_id }}";
                            if (oldValue) {
                                var oldName = "{{ $data->product_category->category_code??''}}" + "-" + "{{$data->product_category->name??'' }}";
                                if (oldName) {
