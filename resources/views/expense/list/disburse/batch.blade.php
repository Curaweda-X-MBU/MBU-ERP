...
<div class="row">
    <div class="col-12">
        <div class="card">
            <form id="paymentBatchForm" action="{{ route('expense.list.payment.batch.add') }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
                            ...
                            <div class="collapse-default">
                                ...
                                <div class="card rounded-lg mb-1 row-scope">
                                    <div class="card-header color-header rounded-lg gray">
                                        <span class="lead collapse-title">Sebanyak {{ count($not_founds) }} ID tidak ditemukan</span>
                                    </div>
                                    <div id="collapsibleInvalid" role="tabpanel" aria-labelledby="heading" class="collapsible collapse" aria-expanded="false">
                                        <div class="card-body p-2 row row-cols-4">
                                            ...
                                            <span>{{ $index + 1 }}. {{ $invalid['id_expense'] }}</span>
                                            @endforeach
                                        </div>
                                    </div>
                                </div>
                                ...
                                @foreach ($payments as $index => $payment)
                                    <div class="card rounded-lg mb-1 row-scope">
                                            @include('expense.list.sections.batch-upload-collapse')
                                    </div>
                                @endforeach
                               {{-- Repeater --}}
                            </div>
...
<script>
    ...
    $(function() {
        $('#allocateButton').on('click', function() {
            ...
            // if there's overpayment left, allocate to last ID
            if (totalOver > 0) {
